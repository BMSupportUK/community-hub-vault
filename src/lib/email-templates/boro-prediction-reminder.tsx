...
const BoroPredictionReminderEmail = ({ displayName, missingCount, nextKickoffAt, predictionsUrl }: Props) => {
  const count = missingCount ?? 1
  const headline = count > 1
    ? `You have ${count} Boro matches still to predict`
    : 'You have a Boro match still to predict'
  return (
...
export const template = {
  component: BoroPredictionReminderEmail,
  subject: (data: Record<string, any>) => {
    const c = data?.missingCount ?? 1
    return c > 1
      ? `Reminder: ${c} Boro matches still to predict`
      : 'Reminder: 1 Boro match still to predict'
  },
  displayName: 'Boro predictions reminder',
  previewData: {
    displayName: 'Alex',
    missingCount: 3,
