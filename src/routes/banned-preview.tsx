...
export const Route = createFileRoute("/banned-preview")({
  component: BannedPreview,
});
    ...
    <div className="fixed inset-0 overflow-hidden bg-black">
      ...
      {open && (
        <div className="fixed inset-0 z-50 grid place-items-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl border border-red-500/30 bg-zinc-950/95 shadow-2xl overflow-hidden">
            ...
            <footer className="px-5 py-3 border-t border-white/10 flex items-center justify-end gap-2">
              <button type="button" onClick={() => setOpen(false)} className="text-sm px-3 py-2 rounded-lg text-white/70 hover:text-white">
                Cancel
              </button>
              <button className="text-sm px-4 py-2 rounded-lg font-semibold text-white bg-gradient-to-r from-violet-600 to-blue-600 shadow-[0_4px_20px_rgba(220,38,38,0.4)]">
                Submit appeal
              </button>
            </footer>
          </div>
