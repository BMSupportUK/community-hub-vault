...
import { XCircle, LogOut, MessageSquarePlus, X } from "lucide-react";
import { useState } from "react";
import { supabase } from "@/integrations/supabase/client";
import { useAuth } from "@/hooks/use-auth";
import { toast } from "sonner";

export const Route = createFileRoute("/_authenticated/account-rejected")({
...
function AccountRejectedPage() {
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-black">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(244,114,182,0.22),transparent_65%)]" />

      <div className="relative z-10 min-h-screen flex flex-col items-center justify-center px-6 text-center">
        <div className="size-20 rounded-full bg-gradient-to-br from-fuchsia-600 to-amber-500 grid place-items-center shadow-[0_0_60px_rgba(244,114,182,0.55)] ring-2 ring-fuchsia-400/40 mb-6">
        ...
        <div className="mt-8 flex flex-col sm:flex-row items-center gap-3">
          <button
            ...
            className="inline-flex items-center gap-2 px-6 py-3 rounded-lg font-semibold text-white bg-gradient-to-r from-fuchsia-600 to-amber-500 hover:from-fuchsia-500 hover:to-amber-400 shadow-[0_8px_30px_rgba(244,114,182,0.45)] transition-all"
          >
      ...
      {open && (
        <div className="fixed inset-0 z-50 grid place-items-center p-4 bg-black/70 backdrop-blur-sm">
          <form
            ...
            className="w-full max-w-lg rounded-2xl border border-fuchsia-500/30 bg-zinc-950/95 shadow-2xl overflow-hidden"
          >
            <header className="h-14 px-5 flex items-center justify-between border-b border-white/10">
              <div className="flex items-center gap-2">
                <div className="size-8 rounded-full bg-gradient-to-br from-fuchsia-600 to-amber-500 grid place-items-center">
            ...
            <div className="p-5 space-y-4">
              <div className="rounded-lg bg-fuchsia-500/10 border border-fuchsia-500/20 p-3 text-xs text-fuchsia-100/90">
                Your appeal will be tagged with reference <span className="font-mono font-semibold">APPEAL</span> and reviewed by a moderator.
              </div>
              <div>
                <label className="block text-xs uppercase tracking-wider text-fuchsia-300/80 mb-2">
                  Explain your appeal
                </label>
                <textarea
                  value={text}
                  ...
                  minLength={10}
                  maxLength={1000}
                  placeholder="Why should we reconsider your account?"
                  className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2.5 text-sm text-white placeholder:text-white/40 outline-none focus:border-fuchsia-500/50 resize-none"
                  autoFocus
                />
            ...
            <footer className="px-5 py-3 border-t border-white/10 flex items-center justify-end gap-2">
              ...
              <button
                ...
                className="text-sm px-4 py-2 rounded-lg font-semibold text-white bg-gradient-to-r from-fuchsia-600 to-amber-500 hover:from-fuchsia-500 hover:to-amber-400 disabled:opacity-50 shadow-[0_4px_20px_rgba(244,114,182,0.4)]"
              >
