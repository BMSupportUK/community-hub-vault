    ...
    <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (v) loadList(user.id); }}>
      <DialogTrigger asChild>
        ...
        >
          ...
          {count > 0 && (
            <span className="absolute -top-1 -right-1 min-w-[20px] h-5 px-1 rounded-full bg-indigo-500 text-white text-[10px] font-bold grid place-items-center ring-2 ring-rail">
              {count > 99 ? "99+" : count}
            </span>
          )}
        </button>
