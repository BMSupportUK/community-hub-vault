...
CREATE OR REPLACE FUNCTION public.fan_zone_moderator_avatar()
RETURNS text LANGUAGE sql IMMUTABLE SET search_path = public
AS $$ SELECT '/__l5e/assets-v1/0b20a840-286f-49d5-a32d-dd65f126266b/moderator-avatar.png'::text $$;

CREATE OR REPLACE FUNCTION public.fan_zone_avatar_locked(_user_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT public.has_any_role(_user_id, ARRAY['admin'::app_role,'management'::app_role,'staff'::app_role,'moderator'::app_role,'boro_fan_zone_moderator'::app_role])
$$;

GRANT EXECUTE ON FUNCTION public.fan_zone_avatar_locked(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.fan_zone_moderator_avatar() TO authenticated, anon;

-- Set the moderator badge avatar for every moderator
UPDATE public.fan_zone_members m
SET fan_avatar_url = public.fan_zone_moderator_avatar()
WHERE public.has_any_role(m.user_id, ARRAY['moderator'::app_role,'boro_fan_zone_moderator'::app_role]);

INSERT INTO public.fan_zone_members (user_id, status, fan_avatar_url, decided_at)
SELECT DISTINCT ur.user_id, 'approved'::fan_zone_status, public.fan_zone_moderator_avatar(), now()
FROM public.user_roles ur
WHERE ur.role IN ('moderator'::app_role,'boro_fan_zone_moderator'::app_role)
  AND NOT EXISTS (SELECT 1 FROM public.fan_zone_members m WHERE m.user_id = ur.user_id)
...
AS $function$
DECLARE _is_staff boolean; _locked boolean; _is_mod boolean; _existing text; _final_avatar text;
BEGIN
