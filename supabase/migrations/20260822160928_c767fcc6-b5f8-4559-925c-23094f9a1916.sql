...
BEGIN
  ...
  -- Reference points for each player's match line, scored as a starter so the
  -- popup breakdown matches the pitch view.
  ...
  FOR v_squad IN SELECT * FROM fantasy_squads WHERE gameweek_id = _gameweek_id LOOP
    ...
    -- Transfer penalties come off the gameweek total (and therefore the leaderboard).
