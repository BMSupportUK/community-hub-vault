-- 1) Remove the older duplicate scoring helpers: with three overloads that all
-- accept four arguments (later ones via defaults), every call became ambiguous
-- and blocked live score updates.
DROP FUNCTION IF EXISTS public.wc_calc_points(integer, integer, integer, integer);
DROP FUNCTION IF EXISTS public.wc_calc_points(integer, integer, integer, integer, text);
...
BEGIN
  ...
  UPDATE public.boro_predictions
     SET points = public.wc_calc_points(home_pred, away_pred, hs, as_, NULL::text, NULL::text),
         updated_at = now()
   WHERE fixture_id = _fixture_id;
END;
$function$;

-- 2) Fantasy scoring referenced a column name that does not exist (vice_captain_id).
...
-- 3) Automated match-day edits have no signed-in user, so the edit history
-- insert failed. Fall back to the post's author.
...
BEGIN
  IF NEW.body IS DISTINCT FROM OLD.body THEN
    ...
    VALUES (OLD.id, COALESCE(auth.uid(), OLD.author_id), OLD.body);
    NEW.edited_at := now();
    NEW.edited_by := COALESCE(auth.uid(), OLD.author_id);
  END IF;
  RETURN NEW;
END $function$;