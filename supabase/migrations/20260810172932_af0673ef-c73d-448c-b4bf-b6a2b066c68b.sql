    ...
    LOOP
      IF v_pick.minutes > 0 THEN
        ...
        IF NOT v_pick.is_starter AND v_pick.minutes < 60 THEN
          -- swap the starter appearance point for sub points
          v_pts := v_pts - 1
                 + 1                                                              -- came off the bench
                 + (CASE WHEN v_pick.minutes >= 30 THEN 1 ELSE 0 END)             -- 30+ minutes
                 + (CASE WHEN (v_pick.goals + v_pick.assists) > 0 THEN 1 ELSE 0 END); -- goal or assist
        END IF;
